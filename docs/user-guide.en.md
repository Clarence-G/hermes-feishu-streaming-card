...
`service.manager: auto` installs and enables an owned persistent `systemd-user` unit when the Linux user manager is available, or falls back to `detached`; it never silently enters `systemd-system` or invokes sudo. The persistent unit starts with the user manager and creates a fresh private process token/pidfile after a systemd restart or host reboot. `stop` keeps it enabled, while `uninstall` disables and removes only the marker-owned unit. An administrator can explicitly configure login lingering when a headless server must start it before login; HFC never runs `loginctl enable-linger`. `systemd-system` remains an explicit Linux-only transient-unit opt-in. Docker stays an ordinary container process with `detached`. See [V4.1 safety controls and troubleshooting](wiki/v4.1-safety-controls.md) for the full boundary.
