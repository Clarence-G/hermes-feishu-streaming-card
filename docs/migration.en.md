...
`service.manager: auto` chooses only an enabled persistent `systemd-user` unit or `detached`; it never silently enters `systemd-system` or invokes sudo. After upgrade, the next official setup/start migrates either a still-running legacy transient service or a dead transient pidfile left by a host reboot. An unverified same-name unit or a still-live PID without matching health remains fail closed. `stop` does not disable startup; `uninstall` removes the managed unit. HFC never enables login lingering automatically; starting before any login is an explicit administrator configuration. `systemd-system` remains an explicit Linux transient-unit opt-in. Docker remains an ordinary container process with `detached`. Hermes 0.19.0 / `v2026.7.20` uses AST-owned run + Base hooks. A legacy run-only manifest gains the Base backup/patch and migrates to v2 only under strict evidence; runtime monitoring and strict repair handle upgrade replacement without an import-hook bridge.
